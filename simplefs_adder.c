...
int filename_exists(FILE *image, const char *filename)
{
    ...
    /* TODO: STUDENT CODE START */
    long total_entries = BLOCK_SIZE / sizeof(dirent_t);
    for (long i = 0; i < total_entries; i++) {
        long pos = ((long)ROOT_DATA_BLOCK * BLOCK_SIZE) + (i * sizeof(dirent_t));
...
int find_free_directory_entry(FILE *image)
{
    ...
    /* TODO: STUDENT CODE START */
    long total_entries = BLOCK_SIZE / sizeof(dirent_t);
    for (long i = 2; i < total_entries; i++) {
        long pos = ((long)ROOT_DATA_BLOCK * BLOCK_SIZE) + (i * sizeof(dirent_t));
        fseek(image, pos, SEEK_SET);
        if (fread(&entry, sizeof(entry), 1, image) != 1) break;
        if (entry.inode_no == 0) {
...
int main(int argc, char *argv[])
{
    ...
    if (strlen(source_name) > 58) {
        printf("Error: file name is too long for SimpleFS (max 58 characters).\n");
        fclose(source); fclose(image); return 1;
    }
